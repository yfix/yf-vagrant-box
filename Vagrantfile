...
Vagrant.configure("2") do |config|
  ...
  config.vm.network "forwarded_port", guest: 80, host: 20180
  config.vm.network "forwarded_port", guest: 22, host: 20122
  ...
  if defined?(VagrantPlugins::HostsUpdater)
    ...
    paths = Dir[File.join(vagrant_dir, 'www', '**', 'yf-box-hosts')]

    # Parse the found yf-box-hosts files for host names.
