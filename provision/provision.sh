...
# up as an 'Internet Site' with the host name 'yf-box'. Note that if your current
# Internet connection does not allow communication over port 25, you will not be
# able to send mail, even with postfix installed.
echo postfix postfix/main_mailer_type select Internet Site | debconf-set-selections
echo postfix postfix/mailname string yf-box | debconf-set-selections
# Disable ipv6 as some ISPs/mail servers have problems with it
echo "inet_protocols = ipv4" >> /etc/postfix/main.cf

# Provide our custom apt sources before running `apt-get update`
ln -sf /srv/config/apt-source-append.list /etc/apt/sources.list.d/yf-box-sources.list
...
if [[ $ping_result == "Connected" ]]; then
	...
	if [[ -n "$(composer --version --no-ansi | grep 'Composer version')" ]]; then
		...
		COMPOSER_HOME=/usr/local/src/composer composer -q global require --no-update --no-dev phpunit/phpunit:4.3.*
...
if [[ ! -e /etc/nginx/server.key ]]; then
	...
	yfboxgenrsa="$(openssl genrsa -out /etc/nginx/server.key 2048 2>&1)"
	echo "$yfboxgenrsa"
fi
...
if [[ ! -e /etc/nginx/server.crt ]]; then
	...
	yfboxsigncert="$(openssl x509 -req -days 365 -in /etc/nginx/server.csr -signkey /etc/nginx/server.key -out /etc/nginx/server.crt 2>&1)"
	echo "$yfboxsigncert"
fi
...
cp /srv/config/init/yf-box-start.conf /etc/init/yf-box-start.conf

echo " * Copied /srv/config/init/yf-box-start.conf               to /etc/init/yf-box-start.conf"
...
# If a bash_prompt file exists in the yf-box config/ directory, copy to the VM.
...
find /etc/nginx/custom-sites -name 'yf-box-auto-*.conf' -exec rm {} \;

# Look for site setup scripts
for SITE_CONFIG_FILE in $(find /srv/www -maxdepth 5 -name 'yf-box-init.sh'); do
	DIR="$(dirname $SITE_CONFIG_FILE)"
	(
		cd "$DIR"
		source yf-box-init.sh
	)
done

# Look for Nginx vhost files, symlink them into the custom sites dir
for SITE_CONFIG_FILE in $(find /srv/www -maxdepth 5 -name 'yf-box-nginx.conf'); do
	DEST_CONFIG_FILE=${SITE_CONFIG_FILE//\/srv\/www\//}
	DEST_CONFIG_FILE=${DEST_CONFIG_FILE//\//\-}
	DEST_CONFIG_FILE=${DEST_CONFIG_FILE/%-yf-box-nginx.conf/}
	DEST_CONFIG_FILE="yf-box-auto-$DEST_CONFIG_FILE-$(md5sum <<< "$SITE_CONFIG_FILE" | cut -c1-32).conf"
	# We allow the replacement of the {yf_box_path_to_folder} token with
	# whatever you want, allowing flexible placement of the site folder
	# while still having an Nginx config which works.
	DIR="$(dirname $SITE_CONFIG_FILE)"
	sed "s#{yf_box_path_to_folder}#$DIR#" "$SITE_CONFIG_FILE" > /etc/nginx/custom-sites/"$DEST_CONFIG_FILE"
done

# Parse any yf-box-hosts file located in www/ or subdirectories of www/
# for domains to be added to the virtual machine's host file so that it is
# self aware.
#
# Domains should be entered on new lines.
echo "Cleaning the virtual machine's /etc/hosts file..."
sed -n '/# yf-box-auto$/!p' /etc/hosts > /tmp/hosts
mv /tmp/hosts /etc/hosts
echo "Adding domains to the virtual machine's /etc/hosts file..."
find /srv/www/ -maxdepth 5 -name 'yf-box-hosts' | \
while read hostfile; do
	while IFS='' read -r line || [ -n "$line" ]; do
		if [[ "#" != ${line:0:1} ]]; then
			if [[ -z "$(grep -q "^127.0.0.1 $line$" /etc/hosts)" ]]; then
				echo "127.0.0.1 $line # yf-box-auto" >> /etc/hosts
...
echo "For further setup instructions, visit http://yf-box.dev"
